...
const REG_PAGES = ['regulations.html', 'Regulation_Signalling.html', 'Regulation_Track-Sizing.html', 'Regulation_Train-Sizing.html'];
const GUIDE_PAGES = ['Guides.html', 'Guide_Schedules.html', 'Guide_Display-Boards.html'];
...
function NavDropdown({ id, label, pages, links, route, linkClass, go }) {
  const isActiveGroup = pages.indexOf(route) !== -1;
  const [open, setOpen] = useState(isActiveGroup);

  useEffect(() => {
    setOpen(isActiveGroup);
  }, [route]);

  return (
    <div className={'nav-dropdown' + (open ? ' open' : '')} id={id}>
      <button
        type="button"
      ...
      <div className="nav-dropdown-menu">
        {links.map(([file, text]) => (
          <a key={file} href={file} className={linkClass(file)} onClick={(e) => go(e, file)}>{text}</a>
        ))}
      </div>
    </div>
  );
    ...
    <header className="site-header">
      ...
      <nav className="main-nav">
        ...
        <NavDropdown
          ...
          go={go}
          links={[
            ['regulations.html', 'Overview'],
            ['Regulation_Signalling.html', 'Signalling'],
            ['Regulation_Track-Sizing.html', 'Track Sizing'],
            ['Regulation_Train-Sizing.html', 'Train Sizing'],
          ]}
        />
        <NavDropdown
          id="guidesDropdown"
          ...
          go={go}
          links={[
            ['Guides.html', 'Overview'],
            ['Guide_Schedules.html', 'Schedules'],
            ['Guide_Display-Boards.html', 'Display Boards'],
          ]}
        />
